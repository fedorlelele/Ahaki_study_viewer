...
- 通常用と点字用の正答を分け、`answer_text` に原記録、`answer_variants` に媒体別正答、`answer_notes` に原注記を保持します。Viewerの表示・採点・コピー・印刷は通常用の正答のみを使用し、点字用の注記は表示しません。範囲外や矛盾した正答は公開JSON生成と検索・監査で拒否します。
...
- 通常解説の確認状態は `ai`（ラベルなし）、`ai_fact_checked`（AI検証済み）、`teacher_approved`（教師承認済み）、`teacher_edited`（教師編集済み）です。AI検証済みはAIによるファクトチェックを完了した解説に使います。2026-09-09以前の教師ラベルは実際の運用に合わせて一度だけAI検証済みに移行し、以後の教師ラベルは人による確認・編集に使います。
