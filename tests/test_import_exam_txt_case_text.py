...
class ExamTxtCaseTextTests(unittest.TestCase):
    def setUp(self):
        self.temp_dir = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp_dir.cleanup)
        self.tmp_path = Path(self.temp_dir.name)

    def test_parse_patient_context_without_shourei_keyword(self):
    ...
    def test_parse_shared_context_without_patient_or_shourei_keyword(self):
        tmp_path = self.tmp_path
        path = write_exam_file(
            tmp_path,
            "A14.txt",
    ...
    def test_parse_combo_choices_inserts_missing_separator(self):
        tmp_path = self.tmp_path
        path = write_exam_file(
            tmp_path,
            "B34.txt",
