...
`markdown.js` は解説の表・見出し・リンク・日本語の強調を描画します。HTMLを無害化し、深掘り解説では目次とキーボード操作できる横スクロール表を付けます。外部リンクは別タブで開きます。

強調は日本語・括弧・英字1文字に接する `**...**` と、記号内に余分な空白がある既存原稿にも対応します。コード内やエスケープされた記号は維持します。数式は `$...$`・`$$...$$`・`\(...\)`・`\[...\]` をMarkdownの解析時に分離し、KaTeXのMathML出力で描画します（外部フォント不要）。[KaTeXの設定](https://katex.org/docs/options)に従い、外部リソース等を許可する `trust` はfalse、マクロ展開回数・サイズは制限し、出力も無害化します。ライブラリの取得失敗や未対応の数式は元の記法で表示を続けます。保存済み本文は書き換えず、既存JSONの制御文字に変換された `\beta`・`\times` だけ表示時に復元します。

深掘り解説の試作は `npm ci` 後に `python3 scripts/preview_deep_dive.py` を起動し、表示されたURLで確認できます。実際の通常ビューとローカルの問題データを使い、A34-029の解説だけを `web_app/content/deep-dives/A34-029.md` に差し替えます。本番サービスへの通信・データ保存は行わず、回答集計は空のテストデータです。別の試作には `--serial`・`--content`・`--model` を指定します。
